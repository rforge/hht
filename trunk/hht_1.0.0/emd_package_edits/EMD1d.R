...
extractimf <- function(residue, tt=NULL, tol=sd(residue)*0.1^2, max.sift=20, 
	...
	#DANIEL BOWMAN'S EDITS JANUARY 16 2013
	#Put max.sift criterion in front, remove max.sift tests from stop rules.
	if(j >=max.sift) {
            imf <- h[,j]
            residue <- residue - imf
            break
	}
	#END DANIEL BOWMAN'S EDITS
        if (stoprule == "type1" && (all(abs(em[,j]) < tol) )) { 
            ...
            if (sum((h[, j-1]-h[, j])^2)/sum(h[, j-1]^2) < tol ) { 
                imf <- h[,j]
                residue <- residue - imf
                break
                }
        } else if (stoprule == "type4") {
             if (sum((em[, j])^2)/sum(h[, j]^2) < tol ) {  
                imf <- h[,j]
                residue <- residue - imf
                break
                }
	#DANIEL BOWMAN'S EDITS, JANUARY 15 2013, modified JANUARY 16 2013
	 } else if (stoprule == "type5" && j >= 2) { #If the number of zero crossings and extrema do not change with successive siftings
             if ((tmp$nextreme+tmp$ncross==prev_excross) && ((tmp$nextreme-tmp$ncross)<=1)) {#If the number of extrema and zero crossings differ by at most one and the total number has not changed
                 s_count=s_count+1
                 if (s_count >= tol) {#If the number of zero crossings and extrema have not changed in TOL iterations, break and return IMF
                     imf <- h[,j]
                     residue <- residue - imf
                     break
                }
...
emd <- function(xt, tt=NULL, tol=sd(xt)*0.1^2, max.sift=20, stoprule="type1", boundary="periodic", 
    ...
    repeat {
        if (j > max.imf) break
        if ((any(j == smlevels)) & (sm == "spline" || sm == "quantile" || sm == "kernel" || sm == "locfit"))
