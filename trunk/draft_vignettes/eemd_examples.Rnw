...
\textbf{WARNING:  The EEMD code in this vignette can take a very long time to run (minutes to hours).}
...
However, this creates a lot more IMFs, and the transient nonlinear signal $a_{1}\sin(\omega_{1}t+a_2\sin(\omega_{2} t))e^{\frac{-(t-50)^{2})}{200}}$ is now distributed across multiple IMFs rather than residing on a single IMF.
...
After EEMD, the spline fitting has been improved and mode mixing has been reduced (Fig. \ref{fig:nonlinstasift}).
The higher frequency signal is still present on several IMFs, but the signal less intermittent than it was in the noisy EMD trial (Fig. \ref{fig:nonlinstarand}).

\subsection{Code}

\textbf{EMD with no added noise}
<<label=nonlinstaemdcode,include=FALSE>>=
library(hht)
dt=0.01
tt=seq_len(10000)*dt
sig=sin(2*pi*tt+0.5*sin(pi*tt))*exp(-(tt-50)^2/100)+sin(pi*tt/10)
emd_result=sig2imf(sig, tt)
plot_imfs(emd_result)
@

\textbf{EMD with added noise}
<<label=nonlinstarandcode,include=FALSE>>=
set.seed(628)
sig2=sin(2*pi*tt+0.5*sin(pi*tt))*exp(-(tt-50)^2/100)+
    sin(pi*tt/10)+rnorm(length(sig), 0, 0.01)
emd_result=sig2imf(sig2, tt)
plot_imfs(emd_result)
@
...
eemd(sig2, tt, noise_amp, trials, nimf, 
    trials_dir = trials_dir, verbose = FALSE)
eemd_result=eemd_compile(trials_dir, trials, nimf)
@

\textbf{Calculate and Plot Averaged IMFs}

<<label=nonlinstasiftcode,include=FALSE>>=
resift_rule="max_var"
resift_result=eemd_resift(eemd_result, 
resift_rule = resift_rule)

time_span=c(0, 100)
imf_list=c(3:7)
os=TRUE
res=FALSE
plot_imfs(resift_result)
@
...
\begin{figure}[ht]
\begin{center}
<<label=nonlinstaemd,fig=TRUE,echo=FALSE>>=
<<nonlinstaemdcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=nonlinstarand,fig=TRUE,echo=FALSE>>=
<<nonlinstarandcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=nonlinstasift,fig=TRUE,echo=FALSE>>=
<<nonlinstasiftcode>>
@
\end{center}
...
The Hilbert spectrogram of this time series should show strong continuous frequency bands at 0.5 Hz and 6 Hz.
...
While IMF 2 still has evidence of mode mixing, IMF 3 is more continuous than it was before.
IMF 5 is much improved compared to the original IMF set.
...
<<label=modemixcode, include=FALSE>>=
library(hht)
set.seed(628)
dt=0.01
#tt=seq_len(2000)*dt
tt = seq_len(200)*dt #For testing
...
<<label=modemixhtcode, include=FALSE>>=
dfreq = 0.1
hgram=hh_render(emd_result, dt, dfreq, verbose = FALSE)
time_span=c(0, 20)
freq_span=c(0, 10)
amp_span=c(0.75, 3)
hhgram_image(hgram, time_span, freq_span, amp_span)
@
...
<<>>=
#trials=100
trials = 2 #For testing
nimf=9
noise_amp=0.4
trials_dir="example_2"
eemd(sig, tt, noise_amp, trials, nimf, 
    trials_dir = trials_dir, verbose = FALSE)
...
<<label=stackedeemdimfscode, include=FALSE>>=
...
resift_rule="max_var"
resift_result=eemd_resift(eemd_result, resift_rule = resift_rule)
...
<<label=stackedeemdhtcode, include=FALSE>>=

freq_span=c(0, 10)
dfreq=0.1
hgram=hh_render(eemd_result, dt, dfreq, 
...
cluster_span = c(0, 100) #For testing
hhgram_image(hgram, time_span = time_span, freq_span = freq_span, 
    amp_span = amp_span, cluster_span = cluster_span)
@ 
...
\begin{figure}[ht]
\begin{center}
<<label=modemix,fig=TRUE,echo=FALSE>>=
<<modemixcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=modemixht,fig=TRUE,echo=FALSE>>=
<<modemixhtcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=stackedeemdimfs,fig=TRUE,echo=FALSE>>=
<<stackedeemdimfscode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=stackedeemdht,fig=TRUE,echo=FALSE>>=
<<stackedeemdhtcode>>
@
\end{center}
...
Although this could have been discovered by using a bandpass filter, the signal would likely have been distorted because of the transient nature of the waveform and the fact that this component has frequency modulation.
...
<<label=pferesiftcode, include=FALSE>>=
resift_rule="max_var"
resift_result=eemd_resift(eemd_result, resift_rule = resift_rule)
time_span=c(5, 10)
imf_list=1:3
...
ftgram_image(sig, dt, ft, time_span = time_span,
...
\begin{figure}[ht]
\begin{center}
<<label=pferesift,fig=TRUE,echo=FALSE>>=
<<pferesiftcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=pfeht,fig=TRUE,echo=FALSE>>=
<<pfehtcode>>
@
\end{center}
...
\begin{figure}[ht]
\begin{center}
<<label=pfeft,fig=TRUE,echo=FALSE>>=
<<pfeftcode>>
@
\end{center}
