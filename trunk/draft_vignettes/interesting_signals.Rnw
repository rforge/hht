...
freq_span = c(0, 30)
hspec=hh_render(emd_result, sdt, dfreq, freq_span, verbose = FALSE)
@
...
freq_span=c(0, 25)
amp_span=c(0, 1)
hhgram_image(hspec, time_span = time_span, freq_span = freq_span, 
amp_span = amp_span, pretty = TRUE, 
img_x_format = "%.0f", img_y_format = "%.0f", 
colorbar_format = "%.0f", trace_format = "%.1f")
@
...
amp_span=c(100,800)
fgram_image(sig, dt, ft, time_span, freq_span, amp_span,
img_x_format = "%.0f", img_y_format = "%.0f",
colorbar_format = "%.0f", trace_format = "%.1f", pretty = TRUE)
@
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=1\textwidth]{stacked1farhspec.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=1\textwidth]{stacked1ft.png}
\end{center}
...
amp_span=c(20, 150)
fgram_image(sig, dt, ft, time_span, freq_span, amp_span,
img_x_format = "%.0f", img_y_format = "%.1f",
colorbar_format = "%.0f", trace_format = "%.1f", pretty = TRUE)
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=1\textwidth]{stacked2imf.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=1\textwidth]{stacked2ht.png}
\end{center}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=1\textwidth]{stacked2ft.png}
\end{center}
...
\textbf{Set up signal, perform EMD and plot IMFs}
<<label=nonlinstaimfs,include=FALSE>>=
library(hht)
dt=0.01
tt = seq(dt, 200, by = dt)
sig=sin(2*pi*tt+0.5*sin(pi*tt/4))*exp(-(tt-100)^2/750)+sin(pi*tt/10)*exp(-(tt-100)^2/500)
...
<<label=nonlinstaht, include=FALSE>>=
time_span=c(40, 160)
freq_span=c(0, 2)
amp_span=c(0.1, 1)
hhgram_image(hspec, time_span = time_span, freq_span = freq_span, amp_span = amp_span,
img_x_format = "%.0f", 
colorbar_format = "%.1f", trace_format= "%.1f",
img_y_format = "%.1f", pretty = TRUE)
@

\textbf{Fourier spectrogram}
<<label=nonlinstaft, include=FALSE>>=
ft=list()
ft$nfft=16384
ft$ns=1000
ft$nov=990
...
freq_span = c(0, 2)
amp_span=c(100,500)
fgram_image(sig, dt, ft, freq_span = freq_span,
amp_span = amp_span, img_x_format = "%.0f",
colorbar_format = "%.0f", trace_format= "%.1f",
img_y_format = "%.1f", pretty = TRUE)
@
...
\begin{figure}[h]
\begin{center}
<<label=nonlinstaimfs,fig=TRUE,echo=FALSE>>=
<<nonlinstaimfs>>
@
\end{center}
...
\begin{figure}[h]
\begin{center}
<<label=nonlinstaht,fig=TRUE,echo=FALSE>>=
<<nonlinstaht>>
@
\end{center}
...
\begin{figure}[h]
\begin{center}
<<label=nonlinstaft,fig=TRUE,echo=FALSE>>=
<<nonlinstaft>>
@
\end{center}
